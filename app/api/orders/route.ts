...
async function ensureOrders() {
  // Pull from Ozon or mock, then upsert minimal fields and geocode lazily client-side (for demo we set fixed coords for AMS)
  ...
  for (const p of postings) {
    ...
    await prisma.order.upsert({
      ...
      create: {
        ...
        ozonOrderId: p.order_id ?? null,
        deliverySchema: 'RFBS',
        sku: p.analytics_data?.sku ?? null,
        article,
        buyerAddressRaw: address,
        lat: 52.3731 + Math.random()*0.05,
        lng: 4.8922 + Math.random()*0.05,
        phoneMasked,
        deliveryPriceRub: price ?? null,
...
export async function GET() {
  ...
  const result = orders.map(o => ({
    id: o.id,
    title: titleFromPosting(o.ozonPostingNumber, o.article ?? undefined),
    lat: o.lat ?? 52.3731, lng: o.lng ?? 4.8922,
    address: o.buyerAddressRaw,
    deliveryDate: o.deliveryDate?.toISOString(),
    phoneMasked: o.phoneMasked ?? undefined,
    priceRub: o.deliveryPriceRub ?? undefined,
    postingNumber: o.ozonPostingNumber,
    article: o.article ?? undefined
  }))
